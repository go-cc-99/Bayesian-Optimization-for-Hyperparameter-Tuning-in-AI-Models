...
def bo_mixed_logei_strategy1_with_trace(
    ...
    *,
    ...
    def row_to_key(row: pd.Series) -> tuple:
        ...
        for name in cols:
            value = row[name]
            if isinstance(param_space[name], tuple):
    ...
    def build_eval_point():
        ...
        def eval_point(row: pd.Series, eval_seed: int):
            ...
            if isinstance(out, dict):
                acc = out.get("acc", out.get("val", out.get("test", np.nan)))
    ...
    seen_keys = {row_to_key(init_used.iloc[i]) for i in range(len(init_used))}
    ...
    for t in tqdm(range(total_needed), desc=f"BO(seed={seed})", disable=not verbose):
        ...
        for retry in range(10):
            rng = np.random.default_rng(seed + 10000 + t * 10 + retry)
            X_cand_unit = random_lhs(candidate_batch, d, rng=rng)
            full_cand_df = map_unit_to_param_df(X_cand_unit, param_space, discrete_mode=discrete_mode)
            keep_idx = [
                idx
                for idx in range(len(full_cand_df))
                if row_to_key(full_cand_df.iloc[idx]) not in seen_keys
            ]
            if keep_idx:
                cand_df = full_cand_df.iloc[keep_idx].reset_index(drop=True)
                break

        if cand_df is None:
            raise RuntimeError("Failed to sample an unseen candidate after 10 retries.")
        ...
        seen_keys.add(row_to_key(picked_row))
